...
object Till {
  def scan(till: Till)(cart: Cart) : (List[String], Int) = {
    ...
    def next(state: TillScannerState, product: Product) : TillScannerState = state.copy(itemsSeen = product :: state.itemsSeen,itemsSeenNotInOffers = product :: state.itemsSeenNotInOffers)
    val finalState = cart.items().foldLeft( init() )( (runningState,product) => next(runningState, product).scan(till)(product) )
